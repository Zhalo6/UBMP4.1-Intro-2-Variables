 ...
 * The maximum value an 8-bit variable can store is 255.
 Some of the benefits to using an 8-bit variable in an 8-bit microcontroller is all the data is sent in one "trip" rather than multiple while still using the maximum capacity.
 ...
 * The advatages to using a constant is that it will never change from its original value.
 By making it into irs own variable it can be used in multiple places much easier.
 ...
 *  It does not reach 50 it reaches around 1-5.
 The program loop has a short delay causing the code to run multiple times per press meaning a single press can register as 10 clicks causing it to light up much before 50.
 ...
 * If LED D4 turns off that means that SW2Count is less than the maxCount.
 As the SW2 button is held SW2Count steadily increases.
 ...
 *The change is that SW2Count no longer resets once it reaches its highest value meaning LED D4 never turns off unless manually reset.
